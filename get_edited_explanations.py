...
def predict(ckpt_path, factual_path, cf_generate_kwargs, dm_name, dm_args, dataloader='train', verbose=True, disable_progress_bar=False, return_tokenizer=False, backwards_compat=False, sparsemap_budget=None):
...
if __name__ == '__main__':
    ...
    factual_path = sys.argv[3] 
    dm_name = sys.argv[4]
    dm_dataloader = "test" if len(sys.argv) <= 5 else sys.argv[5]
    
    backwards_compat = True if "imdb" in dm_name else False
    num_beams = 15
    do_sample = False
    sparsemap_budget = 30 if 'expl' in ckpt_name and 'imdb' in dm_name else None
    ...
    save_edits(
        f'data/edits/{dm_name}_{dm_dataloader}_{sample_mode}_{num_beams}_{ckpt_name}_edited_factual.tsv', 
        out_dict['orig_texts'],
