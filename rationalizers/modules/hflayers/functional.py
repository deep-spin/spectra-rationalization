...
def seq_budget_smap(
    ...
    for h, L in np.ndindex(unary_scores.shape[:2]):
        fg.add(SequenceBudget(u[h,L,:, :], transition_scores[h], budget, force_budget=True))
...
def SparseMAP_exactly_k(scores, k=2):
    fg = TorchFactorGraph()
    u = fg.variable_from(scores)
    for h, L in np.ndindex(scores.shape[:2]):
        fg.add(Budget(u[h, L, :], k, force_budget=True))
    fg.solve(verbose=0)
    return u.value
...
def hopfield_core_forward(query,                           # type: Tensor
    ...
    while update_active_heads.any():
        ...
        num_states = 2
        #Fix this, only works for batch 1
        if Seq_budget:
            transitions = []
            xs = []
            for h in range(num_heads):
                scores = attn_output_weights[h].view(-1)
                ...
                transition_scores = torch.tensor(t[h], device=scores.device)
                transition = torch.zeros(
                    (length + 1, num_states, num_states), device=scores.device
                )
                transition[: length + 1, 0, 0] = (
                    transition_scores[: length + 1]*scaling
                )
                transitions.append(transition.reshape(-1)[2:-2])
                xs.append(x)
            transitions = torch.stack(transitions)
            attn_output_weights = torch.stack(xs).view(num_heads, 1, length, num_states)
        ...
        if xi is None:
            if sparsemap:
                if Seq_budget:
                    if not training:
                        transitions = transitions*scaling
                    xi = seq_budget_smap(
                    attn_output_weights,
                    transitions,
                    budget=budget,
            ...
            if sparsemap:
                if Seq_budget:
                    if not training:
                        transitions = transitions*scaling
                    xi = torch.masked_scatter(input=xi, mask=update_active_heads, source=seq_budget_smap(
                attn_output_weights.masked_select(mask=update_active_heads).view(size=(-1, *xi.shape[1:])), transitions, budget, init,max_iter, step_size))
                else:
