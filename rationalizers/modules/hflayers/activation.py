    ...
    def forward(self,
        ...
        if not self._qkv_same_embed_dim:
            return hopfield_core_forward(
                query=query, key=key, value=value, alpha = self.alpha, sparsemap = self.sparsemap, Seq_budget = self.Seq_budget, budget = self.budget, max_iter = self.max_iter, transitions = self.transitions, init = self.init, step_size = self.step_size, k_ones = self.k, embed_dim_to_check=embed_dim_to_check, num_heads=self.num_heads,
            ...
            return hopfield_core_forward(
                query=query, key=key, value=value, alpha = self.alpha, sparsemap = self.sparsemap, Seq_budget = self.Seq_budget, budget = self.budget, max_iter = self.max_iter, transitions = self.transitions, init = self.init, t = self.t, length = self.length, step_size = self.step_size, k_ones = self.k, embed_dim_to_check=embed_dim_to_check, num_heads=self.num_heads,
                in_proj_weight=self.in_proj_weight, in_proj_bias=self.in_proj_bias, bias_k=self.bias_k,
                bias_v=self.bias_v, add_zero_attn=self.add_zero_attn, dropout_p=self.dropout,
                out_proj_weight=out_weights, out_proj_bias=out_bias, training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights, attn_mask=attn_mask,
