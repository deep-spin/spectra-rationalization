...
from rationalizers.modules.generators import SPECTRAGenerator, HSPECTRAGenerator, HSBSPECTRAGenerator
from rationalizers.modules.predictors import HopfieldSentimentPredictor, SentimentPredictor, SBHopfieldSentimentPredictor
...
class HSPECTRARationalizer(BaseRationalizer):
    """Hopfield SPECTRA Rationalizer"""
    ...
    def __init__(
        ...
        self.emb_path = h_params.get("emb_path", None)
        self.emb_size = h_params.get("emb_size", 300)
        self.K = h_params.get("K", 2)
        self.alpha = h_params.get("alpha", 2.0)
        self.emb_requires_grad = not h_params.get("embed_fixed", True)
        self.hidden_size = h_params.get("hidden_size", 150)
        self.dropout = h_params.get("dropout", 0.5)
        self.bag_dropout = h_params.get("bag_dropout", 0.0)
        self.n_heads = h_params.get("n_heads", 8)
        self.sentence_encoder_layer_type = h_params.get(
            "sentence_encoder_layer_type", "lstm"
        )
        self.transition = h_params.get("transition", 0.0)
        self.budget = h_params.get("budget", 0)
        self.temperature = h_params.get("temperature", 0.01)
        self.factor = h_params.get("factor", "seq-budget")
        # save hyperparams to checkpoint
        self.save_hyperparameters(h_params)
        ...
        if self.factor == "budget":
            self.predictor = HopfieldSentimentPredictor(
                embed=self.emb_layer,
                hidden_size=self.hidden_size,
                output_size=self.nb_classes,
                dropout=self.dropout,
            ...
            self.predictor = SBHopfieldSentimentPredictor(
                embed=self.emb_layer,
                hidden_size=self.hidden_size,
                output_size=self.nb_classes,
                dropout=self.dropout,
